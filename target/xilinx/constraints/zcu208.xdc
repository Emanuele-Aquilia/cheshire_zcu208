...
######################################################################
# Boot Mode Selection (SW15 - GPIO_DIP_SW0/1/2)
######################################################################
set_property -dict {PACKAGE_PIN M18 IOSTANDARD LVCMOS12} [get_ports {boot_mode_i[0]}]
set_property -dict {PACKAGE_PIN L14 IOSTANDARD LVCMOS12} [get_ports {boot_mode_i[1]}]
set_property -dict {PACKAGE_PIN AF15 IOSTANDARD LVCMOS18} [get_ports test_mode_i]

######################################################################
# CPU Reset mapping (Handled by Board Preset)
######################################################################

######################################################################
# UART mapping (FTDI Port C - UART2, bank 65, VCCO 1V2)
######################################################################
set_property -dict {PACKAGE_PIN AR8  IOSTANDARD LVCMOS12} [get_ports uart_rx_i_ftdi]
set_property -dict {PACKAGE_PIN AT9  IOSTANDARD LVCMOS12} [get_ports uart_tx_o_ftdi]
# set_property -dict {PACKAGE_PIN AG14 IOSTANDARD LVCMOS12} [get_ports uart_cts_ni_ftdi]
# set_property -dict {PACKAGE_PIN AL12 IOSTANDARD LVCMOS12} [get_ports uart_rts_no_ftdi]

######################################################################
# QSPI Flash access (via STARTUPE3)
######################################################################
...
# CPU Reset mapping
######################################################################
set_property -dict {PACKAGE_PIN AU1 IOSTANDARD LVCMOS18} [get_ports sys_reset]

######################################################################
# DDR4 C0 Interface (Linked to Board Preset)
######################################################################

######################################################################
# System Clock mapping (Handled by Board Preset)
######################################################################
set_property BOARD_PART_PIN default_sysclk_c0_300mhz_clk_n [get_ports c0_sys_clk_n]
set_property BOARD_PART_PIN default_sysclk_c0_300mhz_clk_p [get_ports c0_sys_clk_p]
...
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets c0_sys_clk_n]
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets c0_sys_clk_p]


# JTAG clock (BSCANE2)
# Both i_tap_dtmcs and i_tap_dmi are driven by the same TCK from the FPGA TAP.
create_clock -period 100.000 -name clk_jtag [get_pins i_cheshire_soc/i_dbg_dmi_jtag/i_dmi_jtag_tap/i_tap_*/INTERNAL_TCK]

# SoC Clock constraints
# set_property CLOCK_DEDICATED_ROUTE BACKBONE [get_nets -of_objects [get_pins i_clkwiz/clk_50]]

########
# UART #
########

# UART speed is at most 5 Mb/s (200ns)
set UART_IO_SPEED 200.0

set_max_delay [expr { $UART_IO_SPEED * 0.35 }] -from [get_ports uart_rx_i_ftdi]
set_false_path -hold -from [get_ports uart_rx_i_ftdi]

set_max_delay [expr { $UART_IO_SPEED * 0.35 }] -to [get_ports uart_tx_o_ftdi]
set_false_path -hold -to [get_ports uart_tx_o_ftdi]

#######
# MIG #
#######

# Dram axi clock : 312.5 MHz (defined by MIG constraints)
set MIG_TCK 3.2

# False-path incoming reset
set MIG_RST_I [get_pins i_dram_wrapper/i_dram/c0_ddr4_aresetn]
...
# SoC, JTAG and MIG clocks are asynchronous
set_clock_groups -asynchronous \
    -group [get_clocks -of_objects [get_pins i_clkwiz/inst/mmcme4_adv_inst/CLKOUT0]] \
    -group [get_clocks -of_objects [get_pins i_dram_wrapper/i_dram/inst/u_ddr4_infrastructure/gen_mmcme4.u_mmcme_adv_inst/CLKOUT0]] \
